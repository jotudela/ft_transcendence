...
class PongGame {
	...
	public leftPaddle1Y: number;
	public leftPaddle1X: number;	
	public leftPaddle2Y: number;
	public leftPaddle2X: number;

	public rightPaddle1Y: number;
	public rightPaddle1X: number;
	public rightPaddle2Y: number;
	public rightPaddle2X: number;

	//boule
	public balls: Ball[];
	public nbBalls: number;
 ...
 *                           Boucle loop
 *========================================================================**/
function drawloop(game : PongGame) 
{
	...
	if (game.nbBalls >= 1){
		drawBall(game, game.balls[0]); updateBall(game, game.balls[0]);}
	if (game.nbBalls >= 2){
		drawBall(game, game.balls[1]); updateBall(game, game.balls[1]);}
...
function handleInput(game: PongGame) {
	//paddle 1
	if (game.keys["q"]) game.leftPaddle1Y -= MV_PADDLE;
	if (game.keys["a"]) game.leftPaddle1Y += MV_PADDLE;
	...
	//paddle 2
	if (game.keys["r"]) game.leftPaddle2Y -= MV_PADDLE;
	if (game.keys["f"]) game.leftPaddle2Y += MV_PADDLE;
	...
	//paddle 3
	if (game.keys["arrowup"]) game.rightPaddle1Y -= MV_PADDLE;
	if (game.keys["arrowdown"]) game.rightPaddle1Y += MV_PADDLE;
	...
	//paddle 4
	if (game.keys["o"]) game.rightPaddle2Y -= MV_PADDLE;
	if (game.keys["l"]) game.rightPaddle2Y += MV_PADDLE;
...
function drawBall(game : PongGame, ball: Ball)
{
	const radius = 7
	game.ctx.beginPath();
...
function updateBall(game : PongGame, ball: Ball)
{
...
function drawFieldRed(ctx: CanvasRenderingContext2D, width: number, height: number) {
	ctx.fillStyle = "#000"; 
...
function drawFieldGreen(ctx: CanvasRenderingContext2D, width: number, height: number) {
	ctx.fillStyle = "#000"; 
	ctx.fillRect(0, 0, width, height);
...
function waitForEnter(game: PongGame): Promise<void> {
